...
const KitchenDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Listen strictly for QR/Table menu changes
  ...
  const toggleTableAvailability = async (id, currentTableStatus) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/menu/${id}/table-availability`, {
        ...
        body: JSON.stringify({ isAvailableForTable: newStatus }) // Specific to the Table menu
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence>
            {displayedItems.map(item => {
              // Read the new Table specific status (defaults to true if undefined)
